    ...
    <div class="d-flex">
        ...
        <div class="container mt-5 m-auto">
            ...
            <div class="mt-5">
                ...
                <!-- show data -->
                <?php
                $query = "SELECT asset_borrow.*, asset.*, user_info.* 
                FROM asset_borrow 
                INNER JOIN asset ON asset_borrow.asset_id = asset.asset_id 
                INNER JOIN user_info ON asset_borrow.emp_id = user_info.user_id 
                WHERE asset_borrow.status = 'request'";
                ...
                if (!$result) {
                    ...
                    // Query execution successful, fetch and display the results
                ?>
                            ...
                            <?php while ($row = mysqli_fetch_assoc($result)) { ?>
                                <tr>
                                    ...
                                    <?php if ($row['status'] == 'request') { ?>
                                        <td>พนักงานรหัส <?php echo $row['emp_id'] ?> ขอยืมอุปกรณ์</td>
                                    <?php } else { ?>
                                        <td>
                                            <font color="white">สามารถขอยืมใช้งานได้</font>
                                        </td>
                                    <?php } ?>
                                    <td>
                                        <button type="button" class="btn btn-primary view_data" data-bs-toggle="modal" data-bs-target="#viewusermodal">
                                            View Details
                                        </button>
                                    </td>
                                </tr>
                            ...
                            <div class="modal-content">
                                <div class="modal-header">
                                    <h1 class="modal-title fs-5" id="staticBackdropLabel">
                                        <font color="black">รายละเอียด การขอยืมอุปกรณ์</font>
                                    </h1>
                                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                                </div>
